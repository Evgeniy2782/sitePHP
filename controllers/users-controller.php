...
if ($requestUri == "/api/users") {
    ...
    if ($requestMethod == "POST") {
        ...
        $name = filter_var($_POST['lastName'] ,FILTER_SANITIZE_STRING);
     

            $user = createUser($login, $password, $name);
...
if (startsWith($requestUri, "/api/users/")) {
    ...
    if ($requestMethod == "POST") {
        ...
        if (!$_FILES["picture"]["error"] == UPLOAD_ERR_NO_FILE) {
            $folder = '/home/evgen/workspace/magazin/uploadsUsers';
            ...
            $file_url = "http://178.72.90.83:8080/uploadsUsers/".$filename;
